    ...
    public class PTTPostService : IFetchData<NewsConditions, Saver>
    {
        ...
        public async Task FetchDataAsync(NewsConditions conditions, Saver saver)
        {
            ...
            foreach (var bordHref in bordsHrefs)
            {
                ...
                do
                {
                    nextPagedUrl = bordpage.QuerySelectorAll("a.btn.wide").Where(x => x.TextContent == "‹ 上頁").First().GetAttribute("href");
                    //已刪除文章
                    ...
                    bordpage = await getDocument($"{platFromUrl}{bordHref}");


                    foreach (var url in postUrls)
                    {
                        #region 標題資料
                        //var pageContent = await getDocument($"https://www.ptt.cc/bbs/Gossiping/M.1623224070.A.3A7.html");
                        //特殊狀況 
